...
\* check it as follows:
\* apalache check --cinit=ConstInit --init=IndInvInit --inv=IndInv \
\*   --length=1 MC_ClockSync7.tla
...
IndInv ==
  \* we do not include TypeInit here,
  \* as apalache would not be able to prove TypeInit as an invariant
  /\ SkewInv
  \* copy & paste from Test2_Inv
  /\ \A p \in Proc:
       state[p] \in { "sent", "sync" } <=>
         \E m \in msgs:
           m.src = p
  \* copy & paste from Test3_Inv
  /\ \A m \in msgs:
  ...
  /\ \A m \in msgs:
      ...
      m.ts >= hc[m.src] + t_max =>
        \A p \in Proc:
          m \in rcvd[p]
  \* copy & paste from Test4_Inv
  /\ \A p \in Proc:
      state[p] = "sync" <=>
        { m.src: m \in rcvd[p] } = Proc \ { p }
  \* no adjustments made before entering "sync"
  /\ \A p \in Proc:
      state[p] /= "sync" => adj[p] = 0
  \* as we see from AdjustClock, here is how 'adj' is computed
  /\ \A p \in Proc:
      state[p] = "sync" => adj[p] = DiffSum(diff, p) \div NProc
  \* we need an invariant about diff
  \* TBD
